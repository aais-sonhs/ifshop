...
class ProductInventoryFlowTests(TestCase):
    ...
    def test_product_table_groups_related_fields_into_compact_columns(self):
        ...
        self.assertEqual(response.status_code, 200)
        for column_key in (
            'product_identity', 'spec_unit', 'category_type', 'import_cost', 'wholesale_prices',
        ):
            self.assertContains(response, f'data-col="{column_key}"')
        for old_column_key in (
            'code', 'name', 'spec', 'unit', 'category', 'product_type',
    ...
    def test_warehouse_page_keeps_warehouse_list_and_adds_inventory_table(self):
        ...
        self.assertContains(response, 'class="text-right th-sortable app-sortable-heading"', count=2)
        self.assertContains(response, 'class="app-sort-toggle-btn warehouse-inventory-sort-btn"', count=2)
        ...
        self.assertContains(response, 'id="inventory_negative_stock_filter"')
        self.assertContains(response, 'function syncWarehouseInventorySortButtons()')
        self.assertContains(response, "currentDirection === 'desc' ? 'asc' : 'desc'")
        self.assertNotContains(response, 'Tăng dần</div>')
        self.assertNotContains(response, 'Giảm dần</div>')
