...
class SalesReportTests(TestCase):
    ...
    def test_finance_report_page_shows_expense_formula_cards(self):
        ...
        self.assertContains(response, '<option value="year">Theo năm</option>', html=True)
        self.assertContains(response, '<option value="range">Theo khoảng ngày</option>', html=True)
        self.assertContains(response, 'id="filter_month"')
        self.assertContains(response, 'id="filter_year"')
        self.assertContains(response, 'id="finance_quick_periods"')
        self.assertContains(response, 'id="finance_period_summary"')
        self.assertContains(response, 'data-range="today"')
        self.assertContains(response, 'data-range="yesterday"')
        self.assertContains(response, 'data-range="7d"')
        self.assertContains(response, 'data-range="30d"')
        self.assertContains(response, 'data-range="current_month"')
        self.assertContains(response, 'data-range="previous_month"')
        self.assertContains(response, 'data-range="current_year"')
    ...
    def test_reports_default_to_full_company_financial_period(self):
        ...
        BusinessConfig.objects.create(
            brand=self.brand,
            business_name='Brand Report',
            financial_period_start_day=start_day,
        )
        ...
        self.assertEqual(
            [row['code'] for row in purchase_payload['data']],
            ['PN-RP-IN-PERIOD'],
        )

        staff_payload = self.client.get(reverse('api_report_staff_sales')).json()
        self.assertEqual(
            [row['salesperson'] for row in staff_payload['staff_data']],
            ['Nhân viên trong kỳ'],
        )
    ...
    def test_sales_report_daily_date_opens_filtered_order_list_in_new_tab(self):
        ...
        self.assertContains(response, '<option value="month" selected>Theo tháng</option>', html=True)
        self.assertContains(response, '<option value="year">Theo năm</option>', html=True)
        self.assertContains(response, '<option value="range">Khoảng ngày</option>', html=True)
        self.assertContains(response, 'id="filter_month"')
        self.assertContains(response, 'id="filter_year"')
        self.assertNotContains(response, 'id="filter_time_group"')
        self.assertNotContains(response, 'Nhóm theo:')
        self.assertContains(response, 'function syncSalesPeriodDates()')
        self.assertContains(response, 'function getAutomaticTimeGroup()')
        self.assertContains(response, "if(periodType==='month') return 'day';")
