...
class SystemManagementScopeTests(TestCase):
    ...
    def test_product_guide_is_available_for_owner_and_superadmin(self):
        ...
        self.assertContains(response, 'Cập nhật nghiệp vụ ngày 21/07/2026')
        ...
        self.assertContains(response, 'Mã đơn bán tạo mới dùng định dạng DH001')
        ...
        self.assertContains(response, 'mã đơn hiển thị bình thường')
        self.assertContains(response, 'Mỗi dòng còn hiển thị tên sản phẩm, Quy cách và Note')
        ...
        self.assertContains(response, 'checkbox căn giữa tại cột Kiểm / ghi chú')
        ...
        self.assertContains(response, 'có thể dùng ô chọn trên từng bảng để xem đến 200 dòng')
        self.assertContains(response, 'mặc định từ ngày đầu tháng đến ngày cuối tháng hiện tại')
        self.assertContains(response, 'bảng chỉ dựng dữ liệu khi mở để tránh làm trang nặng')
