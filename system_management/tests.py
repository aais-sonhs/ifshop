...
from system_management.models import (
    ...
    RoleGroup, ServicePrice, Store, SystemLog, UserProfile,
)
...
class SystemManagementScopeTests(TestCase):
    ...
    def test_superadmin_can_open_platform_management_routes(self):
        ...
        service_price_response = self.client.get(
            reverse('api_get_service_prices'),
    ...
    def test_business_config_remains_under_administration(self):
        ...
        business_config_index = html.index(f'href="{reverse("business_config_tbl")}"')
        self.assertLess(admin_index, settings_index)
        self.assertLess(admin_index, business_config_index)
        self.assertLess(business_config_index, settings_index)
        self.assertIn(
            f'href="{reverse("business_config_tbl")}"',
            html[admin_index:settings_index],
        )
        self.assertContains(response, '<p>Mô hình kinh doanh</p>', html=True)
        self.assertContains(response, 'Ngày bắt đầu kỳ báo cáo')
    ...
    def test_brand_menu_settings_are_superadmin_only(self):
        ...
        api_response = self.client.get(reverse('api_get_brand_menu_settings'))
        self.assertEqual(api_response.status_code, 403)

        save_response = self.client.post(
            reverse('api_save_brand_menu_settings'),
            data=json.dumps({
                'brand_id': self.brand.id,
                'menu_visibility': {'service_prices': False},
            }),
            content_type='application/json',
        )
        ...
        self.client.force_login(self.superuser)
        page_response = self.client.get(reverse('brand_menu_settings'))
        ...
        self.assertNotContains(page_response, 'id="menu_classifications"')
        ...
        self.assertContains(page_response, 'Báo cáo nhân viên bán hàng')

        api_response = self.client.get(reverse('api_get_brand_menu_settings'))
        self.assertEqual(api_response.status_code, 200)
        brands = {row['id']: row for row in api_response.json()['data']}
        self.assertTrue(brands[self.brand.id]['menu_visibility']['service_prices'])
        self.assertTrue(brands[self.brand.id]['menu_visibility']['financial_plans'])
        self.assertTrue(brands[self.other_brand.id]['menu_visibility']['service_prices'])

    def test_superadmin_can_hide_brand_menu_and_service_price_access(self):
        self.client.force_login(self.superuser)
        response = self.client.post(
            reverse('api_save_brand_menu_settings'),
            data=json.dumps({
        ...
        self.client.force_login(self.owner)
        owner_menu = self.client.get(reverse('brand_tbl'))
        self.assertEqual(owner_menu.status_code, 200)
        ...
        self.assertEqual(financial_plan_page.status_code, 302)
        self.assertEqual(financial_plan_page.url, '/dashboard/')
        self.assertEqual(
            self.client.get(reverse('api_get_financial_plans')).status_code,
            403,
        )
        self.assertEqual(
            self.client.post(
        ...
        service_page = self.client.get(reverse('service_price_tbl'))
        self.assertEqual(service_page.status_code, 302)
        self.assertEqual(service_page.url, '/dashboard/')
        self.assertEqual(
            self.client.get(reverse('api_get_service_prices')).status_code,
            403,
        )
        self.assertEqual(
            self.client.post(
                reverse('api_save_service_price'),
                data=json.dumps({'name': 'Gói tháng'}),
                content_type='application/json',
            ).status_code,
            403,
        )
        self.assertEqual(
            self.client.post(
                reverse('api_delete_service_price'),
        ...
        self.client.force_login(self.staff_a)
        staff_menu = self.client.get(reverse('dashboard_page'))
        self.assertEqual(staff_menu.status_code, 200)
        self.assertNotContains(staff_menu, 'href="/product-tbl/"')

        self.client.force_login(self.other_owner)
        other_menu = self.client.get(reverse('brand_tbl'))
    ...
    def test_service_prices_are_scoped_by_brand(self):
        price_a = ServicePrice.objects.create(
            brand=self.brand,
            billing_month=date(2026, 7, 1),
            name='Phí Brand A',
            price=100000,
        )
        price_b = ServicePrice.objects.create(
        ...
        owner_page = self.client.get(reverse('service_price_tbl'))
        ...
        self.assertEqual(
            {row['id'] for row in owner_list.json()['data']},
            {price_a.id},
        )
        self.assertEqual(owner_list.json()['brand']['id'], self.brand.id)
        self.assertEqual(owner_list.json()['data'][0]['billing_month'], '2026-07')
        ...
        self.assertEqual(
            [row['id'] for row in filtered_list.json()['data']],
            [price_a.id],
        )
        self.assertEqual(
            self.client.get(
        ...
        brand_b_list = self.client.get(
            reverse('api_get_service_prices'),
            {'brand_id': self.other_brand.id},
        )
        self.assertEqual(brand_b_list.status_code, 200)
        self.assertEqual(
            {row['id'] for row in brand_b_list.json()['data']},
            {price_b.id},
        )

        super_cross_update = self.client.post(
            reverse('api_save_service_price'),
            data=json.dumps({
                'id': price_a.id,
                'brand_id': self.other_brand.id,
                'billing_month': '2026-07',
                'name': 'Sửa sai phạm vi',
                'price': 1,
            }),
            content_type='application/json',
        )
        self.assertEqual(super_cross_update.status_code, 404)
        price_a.refresh_from_db()
        self.assertEqual(price_a.name, 'Phí Brand A')

    def test_brand_menu_save_rejects_unknown_keys_and_non_boolean_values(self):
        self.client.force_login(self.superuser)
        for visibility in (
            {'unknown_menu': False},
            {'service_prices': 'false'},
        ...
        self.brand.refresh_from_db()
        self.assertEqual(self.brand.menu_visibility, {})

    def test_product_guide_is_available_for_owner_and_superadmin(self):
        response = self.client.get(reverse('product_guide'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Hướng dẫn sử dụng Digimart cho cửa hàng bán lẻ')
        self.assertContains(response, 'Cập nhật 04/08/2026')
        self.assertContains(response, 'Phạm vi hiện tại')
        self.assertContains(response, 'Tài liệu này chỉ trình bày quy trình bán lẻ')
        self.assertContains(response, 'id="guideSearch"')
        self.assertContains(response, 'id="recent-updates"')
        self.assertContains(response, 'Cập nhật nghiệp vụ đến ngày 04/08/2026')
        self.assertContains(response, 'Kỳ báo cáo theo lịch tài chính của công ty')
        self.assertContains(response, 'Quản trị → Mô hình kinh doanh → Tài chính')
        self.assertContains(response, 'tháng 02 là 10/02–09/03')
        self.assertContains(response, 'Cột ghi chú trên danh sách đơn hàng')
        self.assertContains(response, 'Tùy chỉnh hiển thị giống các cột khác')
        self.assertContains(response, 'Phiếu chi mua hàng và khuyến mãi nhà cung cấp')
        self.assertContains(response, 'không còn nút duyệt nhanh ngay tại cột Trạng thái')
        self.assertContains(response, 'Tiền phiếu chi tự tính = Số tiền trước khuyến mãi − Khuyến mãi')
        self.assertContains(response, 'BC Bán hàng bỏ lựa chọn Nhóm theo')
        self.assertContains(response, 'Bắt đầu theo vai trò')
        self.assertContains(response, 'Thiết lập ban đầu')
        self.assertContains(response, 'Quy trình bán lẻ')
        self.assertContains(response, 'Sản phẩm và giá')
        self.assertContains(response, 'Tạo và xử lý đơn bán')
        self.assertContains(response, 'Báo giá → Đơn hàng → Đang đóng gói → Đã xuất kho → Hoàn thành')
        self.assertContains(response, 'không còn bước Đang xử lý')
        self.assertContains(response, 'chọn In → Phiếu đóng hàng A5 sẽ tự chuyển đơn sang Đang đóng gói')
        self.assertContains(response, 'bộ lọc Trạng thái chỉ hiển thị tại tab Tất cả')
        self.assertContains(response, 'Chưa hoàn thành, Đã hoàn thành, Chưa xuất kho, Đã xuất kho và Đang đóng gói')
        self.assertContains(response, 'Thanh toán và công nợ')
        self.assertContains(response, 'Trả hàng và hoàn tiền')
        self.assertContains(response, 'Nhập hàng')
        self.assertContains(response, 'Mã ĐĐH được tự sinh dạng DDH001, DDH002')
        self.assertContains(response, 'mã cũ DDH-001 và mã mới DDH001')
        ...
        self.assertContains(response, 'Báo cáo → BC Kho (/report-inventory/)')
        self.assertContains(response, 'BC nhập xuất tồn theo SP')
        self.assertContains(response, 'BC nhập xuất tồn theo danh mục')
        self.assertContains(response, 'Chọn Từ ngày và Đến ngày')
        ...
        self.assertContains(response, '100.000.000')
        self.assertContains(response, 'Chia đều 12 tháng')
        self.assertContains(response, 'Bước 6 · Khóa kế hoạch sau khi duyệt')
        self.assertContains(response, 'Trạng thái Đã khóa là chế độ chỉ xem')
        self.assertContains(response, 'Phân quyền hệ thống')
        self.assertContains(response, 'Gợi ý cho lần thiết lập đầu tiên')
        self.assertContains(response, '1. Lập ngân sách thu/chi')
        self.assertContains(response, 'Bước 4 · Xếp lịch thanh toán nhà cung cấp')
        self.assertContains(response, 'Ví dụ: lập kế hoạch tiền tháng 08/2026')
        self.assertContains(response, 'Kế hoạch tài chính không thay thế BC Tài chính')
        self.assertContains(response, 'Báo cáo lợi nhuận dự kiến từ báo giá')
        self.assertContains(response, 'Báo cáo → BC LN dự kiến (/report-quotation-profit/)')
        self.assertContains(response, 'Thử CK CTV chỉ mô phỏng trên trình duyệt')
        self.assertContains(response, 'Cảnh báo tồn kho qua email')
        self.assertContains(response, 'Cài đặt → Báo email tồn kho (/setting/stock-alert-email/)')
        self.assertContains(response, 'Mỗi email chỉ nhận các sản phẩm thuộc danh mục được gán')
        ...
        self.assertContains(response, 'dùng công tắc Kế hoạch tài chính để bật hoặc tắt riêng chức năng')
        self.assertContains(response, 'khách thu phí một lần')
        self.assertContains(response, 'thương hiệu A không làm đổi menu của thương hiệu B')
        self.assertContains(
            response,
            'Tổng tiền về lấy các phiếu thu Hoàn thành theo Ngày phiếu thu',
        )
        self.assertContains(response, 'Toàn bộ công thức bán lẻ')
        self.assertContains(response, 'Thành tiền dòng hàng')
        self.assertContains(response, 'Số lượng × Đơn giá − Chiết khấu dòng')
        self.assertContains(response, 'Tổng thanh toán / Doanh thu đơn')
        self.assertContains(response, 'max(Tiền hàng − Chiết khấu chung + Phí vận chuyển + Chi phí khác, 0)')
        self.assertContains(response, 'Còn phải thu trên đơn')
        self.assertContains(response, 'max(Tổng thanh toán − Đã thu, 0)')
        self.assertContains(response, 'Công nợ trên BC Bán hàng')
        self.assertContains(response, 'Doanh thu − Đã thu')
        self.assertContains(response, 'Lợi nhuận theo đơn')
        self.assertContains(response, 'Doanh thu đơn − Tổng giá vốn của đơn')
        self.assertContains(response, 'Lợi nhuận gộp toàn kỳ')
        self.assertContains(response, 'Doanh thu thuần − Giá vốn thuần')
        self.assertContains(response, 'Có thể bán')
        self.assertContains(response, 'Tồn kho thực tế − Số lượng đang giữ cho đơn đủ điều kiện')
        self.assertContains(response, 'Giá trị tồn trên BC Kho')
        self.assertContains(response, 'max(Tồn hiện tại, 0) × Giá vốn')
        self.assertContains(response, 'Tồn cuối kỳ trên BC Nhập xuất tồn')
        self.assertContains(response, 'Tồn đầu kỳ + Nhập trong kỳ − Xuất trong kỳ')
        self.assertContains(response, 'Giá trị xuất trong kỳ')
        self.assertContains(response, 'Giá vốn ghi nhận tại từng giao dịch')
        self.assertContains(response, 'Tổng chi phí trên BC Tài chính')
        self.assertContains(response, 'Chi phí khác + Hàng nhập sau khuyến mãi')
        self.assertContains(response, 'Lãi/Lỗ trên BC Tài chính')
        self.assertContains(response, 'Tổng thu − Tổng chi phí')
        self.assertContains(response, 'id="sales-report-guide"')
        self.assertContains(response, 'Cách đọc Báo cáo bán hàng')
        self.assertContains(response, 'Phạm vi mặc định chỉ lấy đơn')
        self.assertContains(response, '1. Thẻ Tổng quan')
        self.assertContains(response, '2. Bảng Doanh thu theo ngày và Tổng hợp ngày')
        self.assertContains(response, '6. Theo đơn hàng')
        self.assertContains(response, '8. Hàng bán chậm')
        self.assertContains(response, '9. BC Nhân viên bán hàng')
        self.assertContains(response, '10. BC Lợi nhuận dự kiến từ báo giá')
        self.assertContains(response, 'LN dự kiến ÷ DT dự kiến × 100%')
        self.assertContains(response, 'Giá vốn đã chụp / Giá vốn hiện tại / Thiếu giá vốn')
        self.assertContains(response, 'Riêng BC Nhân viên BH')
        self.assertContains(response, 'Bộ lọc Nhóm khách cho phép xem riêng Khách lẻ hoặc Khách buôn / sỉ')
        self.assertContains(response, 'toàn bộ doanh thu, giá vốn, lợi nhuận, tỷ suất, Bonus và tỷ lệ đóng góp được tính lại')
        self.assertContains(response, 'Tất cả / Khách lẻ / Khách buôn / sỉ')
        self.assertContains(response, 'đơn khách vãng lai được tính vào Khách lẻ')
        self.assertContains(response, 'Lợi nhuận gộp ÷ Doanh thu × 100%')
        self.assertContains(
            response,
            'Tỷ suất tổng được tính từ số tổng, không lấy trung bình tỷ suất từng nhân viên.',
        )
        self.assertContains(response, 'Top 3 sản phẩm bán chạy của nhân viên')
        self.assertContains(response, 'Đơn đã xuất kho nhưng còn nợ')
        self.assertContains(response, 'đơn giữ trạng thái này cho đến khi thu đủ tiền')
        self.assertContains(response, 'Checklist đầu ngày, trong ngày và cuối ngày')
        self.assertContains(response, 'Khi số liệu chưa khớp')
        self.assertContains(response, 'Đơn đã xuất kho vẫn có công nợ')
        self.assertContains(response, 'id="faq"')
        self.assertContains(response, 'Câu hỏi thường gặp')
        self.assertContains(response, 'Đơn hàng đã Hoàn thành thì trong kho đã xuất hay chưa xuất?')
        self.assertContains(response, 'Đã xuất kho và tồn kho đã được trừ trước đó')
        self.assertContains(response, 'Hệ thống trừ tồn ngay tại bước Đã xuất kho')
        self.assertContains(response, 'không trừ kho lần thứ hai')
        self.assertContains(response, 'Sản phẩm dịch vụ không trừ kho')
        self.assertContains(response, 'Lịch sử kho của sản phẩm')
        self.assertContains(response, 'Không tự trừ kho thêm vì có thể trừ hai lần')
        self.assertContains(response, '2) Đơn hàng đang ở trạng thái Đã xuất kho, để đơn Hoàn thành cần thêm bước nào?')
        self.assertContains(response, 'Cần thu đủ tiền và duyệt đơn nếu đơn có yêu cầu người duyệt')
        self.assertContains(response, 'Tổng các phiếu thu Hoàn thành')
        self.assertContains(response, 'không trừ tồn kho lần thứ hai')
        self.assertContains(response, '3) Đơn hàng chỉ đang ở trạng thái Đã xuất kho có được tính doanh thu không?')
        self.assertContains(response, 'Báo cáo bán hàng mặc định tính doanh thu của cả đơn Đã xuất kho và Hoàn thành')
        self.assertContains(response, 'Doanh thu được ghi theo Ngày đặt hàng')
        self.assertContains(response, 'Công nợ = Doanh thu − Đã thu')
        self.assertContains(response, 'không làm tăng doanh thu lần thứ hai')
        self.assertContains(response, '4) Quy trình đơn hàng hiện có những trạng thái nào?')
        self.assertContains(response, 'Nếu có dữ liệu cũ từng mang trạng thái Đang xử lý')
        self.assertContains(response, 'không làm mất đơn hay lịch sử')
        self.assertContains(response, 'Khách lẻ / khách vãng lai')
        self.assertNotContains(response, 'F&amp;B / Cafe')
        self.assertNotContains(response, 'Hướng dẫn chi tiết cho spa')
        self.assertNotContains(response, 'Hướng dẫn chi tiết cho nhà thuốc')
        ...
        self.assertContains(super_response, 'Hướng dẫn sử dụng Digimart cho cửa hàng bán lẻ')
